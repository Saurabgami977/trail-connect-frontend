...
export default function UserDropdown() {
  ...
  return (
    <div className="relative">
      ...
      >
        <span className="mr-3 rounded-full overflow-hidden">
          <Image
            width={50}
            height={50}
            style={{
            ...
            src={
              auth?.user?.avatar
                ? process.env.NEXT_PUBLIC_ClOUDFLARE_API + auth?.user?.avatar
                : Owner
            }
            alt="User"
          />
      ...
      >
        <div>
          ...
          <span className="mt-0.5 block text-theme-xs text-gray-500 ">
            {auth?.isAuthenticated ? auth.user.email : "Please sign in"}
          </span>
        </div>
        ...
        <ul className="flex flex-col gap-1 pt-4 pb-3 border-b border-gray-200 ">
          ...
          {/* <li>
            <DropdownItem
              onItemClick={closeDropdown}
              tag="a"
              href={auth?.user?.role === 'admin' ? '/profile' : '/user/profile'}
              className="flex items-center gap-3 px-3 py-2 font-medium text-gray-700 rounded-lg group text-theme-sm hover:bg-gray-100 hover:text-gray-700  "
            >
              <svg
                className="fill-gray-500 group-hover:fill-gray-700 "
